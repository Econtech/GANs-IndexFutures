...
class LossAccPlotter(object):
    ...
    def redraw(self):
        ...
        ncol = 1
        labels = ["$CHART generator", "$CHART discriminator"]
        if self.show_averages:
            labels.extend(["$CHART generator (avg %d)" % (self.averages_period,),
                           "$CHART discriminator (avg %d)" % (self.averages_period,)])
            ncol += 1
        if self.show_regressions:
            labels.extend(["$CHART generator (regression)",
                           "$CHART discriminator (regression)"])
            ncol += 1
    ...
    def _redraw_main_lines(self):
        """Draw the main lines of values (i.e. loss generator, loss val, acc generator, acc val).
        Returns:
        ...
        if ax1:
            ...
            h_lv, = ax1.plot(list(self.values_loss_val.keys()), list(self.values_loss_val.values()),
                             ls_loss_val, label="loss discriminator", alpha=alpha_main)
        ...
        if ax2:
            ...
            h_av, = ax2.plot(list(self.values_acc_val.keys()), list(self.values_acc_val.values()),
                             ls_acc_val, label="acc. discriminator", alpha=alpha_main)
    ...
    def _redraw_averages(self):
        ...
        if ax2:
            ...
            h_av, = ax2.plot(av_sma_x, av_sma_y, self.linestyles["acc_val_sma"],
                             label="acc. discriminator (avg %d)" % (self.averages_period,),
                             alpha=alpha_sma)
            handles.extend([h_at, h_av])
    ...
    def _redraw_regressions(self):
        ...
        if ax1:
            ...
            h_lv, = ax1.plot(lv_regression[0], lv_regression[1],
                             ...
                             label="loss discriminator regression",
                             alpha=alpha_regression)
            handles.extend([h_lt, h_lv])
        if ax2:
            # for accuracy chart
            ...
            h_av, = ax2.plot(av_regression[0], av_regression[1],
                             ...
                             label="acc discriminator regression",
                             alpha=alpha_regression)
            handles.extend([h_at, h_av])
